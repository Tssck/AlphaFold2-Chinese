...
def constrain_force_cycle(uint_crd, scaler, pair_dr, atom_i_serials,
    ...
    Supported Platforms:
        ``GPU``
    """
    test_frc = np.zeros(uint_crd.shape, np.float32)
    ...
    test_frc = ops.tensor_scatter_add(test_frc, np.expand_dims(atom_j_serials, -1), frc_lin)
    test_frc = ops.tensor_scatter_add(test_frc, np.expand_dims(atom_i_serials, -1), -frc_lin)
